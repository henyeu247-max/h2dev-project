...
4. **`final.mp4`** — auto-merged 1920×1080 video assembled by FFmpeg, ready for local QA. Image timing is auto-calculated from word distribution × actual audio length; it is an estimate and is not a publication or policy certificate.
...
No manual timing file. Timing is calculated once, from actual audio length, after voice generation.
The merge command verifies that the output contains both video and audio. Review the
complete MP4 for narration/visual alignment before calling the video finished.
