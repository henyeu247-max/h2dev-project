...
- **Typography ~20 bậc.** 9 bậc dùng được (11/12/13/14/16/18/20/24/30px) + **11 bậc lẻ tùy tiện** (9, 9.5, 10, 10.5, 11.5, 12.5, 13.5, 14.5, 15, 17, 22, 26px). Tailwind arbitrary `text-[Npx]` dùng ~112 lần so với class chuẩn ~152 lần (**~42% tùy ý**); riêng `text-[11px]` xuất hiện 70 lần. `font-weight:650` / `font-extrabold`(800) / `font-black`(900) vượt khai báo `@font-face` (`400 700` / `500 700`) → trình duyệt **synthetic-bold**. **17 giá trị `line-height`** và **14 giá trị `letter-spacing`**, cùng một giá trị viết 3 kiểu (`0.04em` / `.04em` / `0.04em !important`).
