...
- **Tầng 1 — Kho Học Liệu & Âm Thanh Chuẩn:** Catalog 136 video bài giảng kèm phụ đề sạch 3 định dạng (`transcript.json`, `transcript.srt`, `transcript.txt`), Voice DNA Studio với mẫu trích xuất 45s (tính WPM, profile clone giọng), **Kho 38 tracks nhạc nền đã audit Gemini Multimodal (25 SAFE YPP, 9 REVIEW, 4 COPYRIGHTED cấm dùng) tích hợp Trạm phát nhạc nền Interactive Music Studio Modal**, cẩm nang Master SOP, và media đã kiểm định ffprobe (luồng video + audio khác 0 byte).
...
- **Tầng 5 — Hạ Tầng Công Cụ & Mô Hình:** MCP Tool Server chạy 100% LOCAL tại `D:\Mcp-Pool-Vps` dưới dạng Windows Service chính thức (`MCP_Pool_Service`, NSSM `SERVICE_AUTO_START`, :3988/mcp, 180+ tools: vidIQ, Trends, Firecrawl, Exa, Tavily, Playwright...) + **Gemini Multimodal Audio/Video Engine (`scripts/gemini_audio_engine.js` & `gemini_analyze_media`) với pool xoay vòng 4 Live Keys tự động** + AI Chat Model Gateway tại 9Router (:20128). Độc lập 100% ngoài WorkBuddy (0 background tasks trong WorkBuddy).
- **Tầng 6 — Tự Động Hóa & Script Kiểm Định:** Pipeline tiếp nhận (`inbox/`), đồng bộ catalog, **Nguồn số duy nhất `scripts/lib/counts.js` & manifest `data/counts-manifest.json`**, bộ test validation (`validate-project.js`), công cụ sửa chữa và quét bảo mật. Kỷ luật Ephemeral Cleanup (xóa sạch script tạm sau khi check-pass).
...
## PHẦN 8 — BỘ 11 TIÊU CHUẨN VÀNG NGHIỆM THU CHECK PASS 1 HỒ SƠ KÊNH MẪU THỰC CHIẾN (CANONICAL CHANNEL BENCHMARK E2E)

> **Nguyên tắc cốt lõi:** Một kênh mẫu trong kho dữ liệu raw không đơn thuần là một bức ảnh chụp màn hình hay một cái tên. Nó là **một bản thiết kế giải mã toàn diện (Reverse-Engineering Dossier)**. Dù là kênh cũ hay kênh mới, khi nghiệm thu check pass 1 kênh bắt buộc phải kiểm tra chi tiết bằng tay và trình duyệt thật theo đúng 11 tiêu chuẩn vàng sau:
