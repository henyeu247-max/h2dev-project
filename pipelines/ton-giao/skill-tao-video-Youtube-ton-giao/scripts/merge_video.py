...
def compute_timings(sections: list[str], audio_duration: float) -> list[tuple[float, float]]:
    ...
    This is a word-fraction estimate, not a verified speech-to-image alignment.
    The exported MP4 must still be watched/listened to and corrected where the
    narration and visual change do not line up.
    """
...
def validate_output(path: Path) -> None:
    """Fail closed unless the final file is non-empty and has video plus audio."""
    if not path.is_file() or path.stat().st_size < 1024:
        raise SystemExit(f"Final output missing or too small: {path}")
    cmd = ["ffprobe", "-v", "error", "-show_entries", "stream=codec_type", "-of", "json", str(path)]
    try:
        data = json.loads(subprocess.run(cmd, capture_output=True, text=True, check=True, timeout=30).stdout)
    except (OSError, subprocess.SubprocessError, json.JSONDecodeError) as exc:
        raise SystemExit(f"Final output could not be decoded: {exc}")
    types = {stream.get("codec_type") for stream in data.get("streams", [])}
    if not {"video", "audio"}.issubset(types):
        raise SystemExit(f"Final output must contain video and audio streams: {sorted(types)}")
...
def main() -> None:
    ...
    result = {
        ...
        "timing_method": "word_fraction_estimate",
        "qa_required": True,
        "fps": FPS,
